...
export function normalizeDefaultValue(value: string) {
  ...
  if (
    ...
    /^nextval\(.+::regclass\)$/i.test(trimmed) ||
    /^-?\d+(\.\d+)?$/.test(trimmed)
  ) {
    return trimmed
  }
...
export function buildCreateTableColumnDefinition(
  ...
  const typeWithSize = supportsColumnSize(connection, baseType, column.size)
  ...
  if (column.autoIncrement && !supportsAutoIncrement(connection, baseType)) {
    throw new Error(
      connection.databaseType === "postgresql"
        ? "No PostgreSQL, identity/auto increment está disponível apenas para SMALLINT, INTEGER ou BIGINT."
        : "Auto increment não está disponível para o tipo de coluna selecionado."
    )
  }
...
function supportsAutoIncrement(connection: SavedConnection, dataType: string) {
...
function supportsColumnSize(connection: SavedConnection, dataType: string, size: string) {
  if (!size.trim()) {
    return false
  }
...
export function buildCreateTableIndexDefinition(
  ...
  const sqlServerIndexType =
    connection.databaseType === "sqlserver" &&
    /^(CLUSTERED|NONCLUSTERED)$/i.test(index.sqlServerIndexType ?? "")
      ? `${index.sqlServerIndexType?.trim().toUpperCase()} `
      : ""
  ...
  return `CREATE ${uniquePrefix}${sqlServerIndexType}INDEX ${quotedIndexName} ON ${quotedSchema}.${quotedTable} (${quotedColumnNames.join(", ")})`
}
